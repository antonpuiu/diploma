...
namespace Zep
{
    CommandContext::CommandContext(const std::string& commandIn, ZepMode_Vim& md, uint32_t lastK,
                                   uint32_t modifierK, EditorMode editorMode)
        : owner(md)
    ...
    void CommandContext::UpdateRegisters()
    {
        ...
        if (op == CommandOperation::Delete || op == CommandOperation::DeleteLines) {
            ...
            std::string str = std::string(buffer.GetText().begin() + beginRange,
            ...
            while (!registers.empty()) {
                ...
                if (registers.top() >= 'A' && registers.top() <= 'Z') {
                    ...
                    owner.GetEditor().GetRegister((const char)chlow).lineWise =
                        (op == CommandOperation::CopyLines);
                } else {
                    owner.GetEditor().GetRegister(registers.top()).text = str;
                    owner.GetEditor().GetRegister(registers.top()).lineWise =
                        (op == CommandOperation::CopyLines);
                }
                registers.pop();
            }
    ...
    void CommandContext::GetCommandAndCount()
    {
        ...
        if (itr != commandText.end()) {
            if (*itr == 'f' || *itr == 'F' || *itr == 'c') {
                ...
                while (itr != commandText.end() && std::isgraph(*itr) && !std::isdigit(*itr)) {
                    command1 += *itr;
                    itr++;
                }
            }
        ...
        while (itr != commandText.end() && (std::isgraph(*itr) || *itr == ' ')) {
            command2 += *itr;
            itr++;
        }
    ...
    void ZepMode_Vim::ClampCursorForMode()
    {
        ...
        if (m_currentMode == EditorMode::Normal) {
            GetCurrentWindow()->SetBufferCursor(GetCurrentWindow()->GetBuffer().ClampToVisibleLine(
    ...
    bool ZepMode_Vim::GetOperationRange(const std::string& op, EditorMode mode,
                                        BufferLocation& beginRange, BufferLocation& endRange) const
    {
        ...
        if (op == "visual") {
            ...
            beginRange = bufferCursor;
            endRange =
                buffer.ChangeWordMotion(bufferCursor, SearchType::Word, SearchDirection::Forward);
        } else if (op == "cW") {
            beginRange = bufferCursor;
            endRange =
                buffer.ChangeWordMotion(bufferCursor, SearchType::WORD, SearchDirection::Forward);
    ...
    bool ZepMode_Vim::HandleExCommand(const std::string& strCommand, const char key)
    {
        ...
        if (key == ExtKeys::RETURN) {
            ...
            if (GetEditor().Broadcast(
                    std::make_shared<ZepMessage>(Msg::HandleCommand, strCommand))) {
                m_currentCommand.clear();
                return true;
                ...
                if (strTok.size() > 1) {
                    if (strTok[1] == "%") {
                        pTab->AddWindow(
                            &GetEditor().GetActiveTabWindow()->GetActiveWindow()->GetBuffer(),
                            pWindow, true);
                    } else {
                        auto fname = strTok[1];
                        auto pBuffer = GetEditor().GetFileBuffer(fname);
                    ...
                    pTab->AddWindow(
                        &GetEditor().GetActiveTabWindow()->GetActiveWindow()->GetBuffer(), pWindow,
                        true);
                }
    ...
    bool ZepMode_Vim::GetCommand(CommandContext& context)
    {
        ...
        // Moving between tabs
            ...
            auto lineEnd =
                context.buffer.GetLinePos(context.bufferCursor, LineLocation::LineLastNonCR);
            GetCurrentWindow()->SetBufferCursor(
                std::min(context.bufferCursor + context.count, lineEnd));
            context.commandResult.flags |= CommandResultFlags::HandledCount;
            return true;
        } else if (context.command == "h" || context.lastKey == ExtKeys::LEFT) {
            auto lineStart =
                context.buffer.GetLinePos(context.bufferCursor, LineLocation::LineBegin);
            GetCurrentWindow()->SetBufferCursor(
                std::max(context.bufferCursor - context.count, lineStart));
            context.commandResult.flags |= CommandResultFlags::HandledCount;
            return true;
        } else if ((context.command == "f" && (context.modifierKeys & ModifierKey::Ctrl)) ||
            ...
            context.commandResult.flags |= CommandResultFlags::HandledCount;
            return true;
        } else if ((context.command == "d" && (context.modifierKeys & ModifierKey::Ctrl)) ||
                   context.lastKey == ExtKeys::PAGEDOWN) {
            ...
            context.commandResult.flags |= CommandResultFlags::HandledCount;
            return true;
        } else if ((context.command == "b" && (context.modifierKeys & ModifierKey::Ctrl)) ||
            ...
            context.commandResult.flags |= CommandResultFlags::HandledCount;
            return true;
        } else if ((context.command == "u" && (context.modifierKeys & ModifierKey::Ctrl)) ||
                   context.lastKey == ExtKeys::PAGEUP) {
            GetCurrentWindow()->MoveCursorY(-(GetCurrentWindow()->GetNumDisplayedLines() / 2) *
                                            context.count);
            context.commandResult.flags |= CommandResultFlags::HandledCount;
            ...
            auto target = context.buffer.WordMotion(context.bufferCursor, SearchType::Word,
            ...
            auto target = context.buffer.EndWordMotion(context.bufferCursor, SearchType::Word,
                                                       SearchDirection::Forward);
            GetCurrentWindow()->SetBufferCursor(target);
            return true;
        } else if (context.command == "E") {
            ...
            context.beginRange =
                context.buffer.GetLinePos(context.bufferCursor, LineLocation::LineCRBegin);
            context.endRange =
                context.buffer.GetLinePos(context.bufferCursor, LineLocation::BeyondLineEnd);
            context.op = CommandOperation::Delete;
            ...
            if (m_currentMode == EditorMode::Visual) {
                ...
                if (context.command == "V") {
                    m_visualBegin =
                        context.buffer.GetLinePos(context.bufferCursor, LineLocation::LineBegin);
                    m_visualEnd = context.buffer.GetLinePos(context.bufferCursor,
            ...
            if (m_currentMode == EditorMode::Visual) {
                ...
                if (context.command != "x" || std::isgraph(context.buffer.GetText()[loc]) ||
                    std::isblank(context.buffer.GetText()[loc])) {
                    context.beginRange = loc;
                    context.endRange =
                        std::min(context.buffer.GetLinePos(loc, LineLocation::LineCRBegin),
            ...
            if (context.command == "d") {
                ...
                if (GetOperationRange("visual", context.mode, context.beginRange,
                                      context.endRange)) {
                    context.op = CommandOperation::Delete;
                    context.commandResult.modeSwitch = EditorMode::Normal;
                } else {
        ...
        // Substitute
            ...
            if (context.command == "S") {
                ...
                else if (GetOperationRange("cursor", context.mode, context.beginRange,
                                           context.endRange)) {
                    context.op = CommandOperation::Delete;
                }
            } else {
            ...
            if (context.command == "c") {
                ...
                if (context.command.length() == 3) {
                    ...
                    context.endRange = buffer.FindOnLineMotion(
                        bufferCursor, (const utf8*)&context.command[2], SearchDirection::Forward);
                    context.op = CommandOperation::Delete;
                } else {
                    context.commandResult.flags |= CommandResultFlags::NeedMoreChars;
                }
            ...
            if (!context.pRegister->text.empty()) {
                if (context.pRegister->lineWise) {
                    ...
                    context.beginRange = context.buffer.LocationFromOffsetByChars(
                        context.bufferCursor, 1, LineLocation::LineCRBegin);
                }
                context.op = CommandOperation::Insert;
            }
        } else if (context.command == "P") {
            ...
            GetCurrentWindow()->SetBufferCursor(
                context.buffer.GetLinePos(bufferCursor, LineLocation::LineCRBegin));
            context.commandResult.modeSwitch = EditorMode::Insert;
            return true;
        } else if (context.command == "I") {
            ...
            if (!m_lastFind.empty()) {
                GetCurrentWindow()->SetBufferCursor(context.buffer.FindOnLineMotion(
                    bufferCursor, (const utf8*)m_lastFind.c_str(), m_lastFindDirection));
                return true;
            }
        } else if (context.command[0] == 'f') {
            if (context.command.length() > 1) {
                GetCurrentWindow()->SetBufferCursor(context.buffer.FindOnLineMotion(
                    bufferCursor, (const utf8*)&context.command[1], SearchDirection::Forward));
            ...
            if (context.command.length() > 1) {
                GetCurrentWindow()->SetBufferCursor(context.buffer.FindOnLineMotion(
                    bufferCursor, (const utf8*)&context.command[1], SearchDirection::Backward));
                m_lastFind = context.command[1];
            ...
            if (context.mode == EditorMode::Normal) {
                ...
                GetCurrentWindow()->SetBufferCursor(context.buffer.GetLinePos(
                    GetCurrentWindow()->GetBufferCursor(), LineLocation::LineBegin));
                return true;
            }
            return false;
        } else {
        ...
        if (context.op == CommandOperation::Delete || context.op == CommandOperation::DeleteLines) {
            auto cmd = std::make_shared<ZepCommand_DeleteRange>(
                context.buffer, context.beginRange, context.endRange, context.bufferCursor);
            ...
            auto cmd = std::make_shared<ZepCommand_Insert>(context.buffer, context.beginRange,
        ...
        if (m_currentMode == EditorMode::Visual) {
            ...
            if (m_lineWise) {
                m_visualEnd = GetCurrentWindow()->GetBuffer().GetLinePos(
                    GetCurrentWindow()->GetBufferCursor(), LineLocation::BeyondLineEnd);
            } else {
                m_visualEnd = GetCurrentWindow()->GetBuffer().LocationFromOffsetByChars(
            ...
            if (GetCommand(context)) {
                ...
                if (!(context.commandResult.flags & CommandResultFlags::HandledCount)) {
                    for (int i = 1; i < context.count; i++) {
                        ...
                        CommandContext contextInner(m_currentCommand, *this, key, modifierKeys,
                                                    m_currentMode);
                        if (GetCommand(contextInner) && contextInner.commandResult.spCommand) {
                            // Group counted
                            if (i == (context.count - 1) && !appendDotInsert) {
                                contextInner.commandResult.spCommand->SetFlags(
                ...
                if (appendDotInsert) {
                    if (!m_lastInsertString.empty()) {
                        auto cmd = std::make_shared<ZepCommand_Insert>(
                            GetCurrentWindow()->GetBuffer(), GetCurrentWindow()->GetBufferCursor(),
                            m_lastInsertString, context.bufferCursor);
                ...
                if (!m_currentCommand.empty() &&
                    m_currentCommand.find_first_not_of("0123456789") == std::string::npos) {
                    context.commandResult.flags |= CommandResultFlags::NeedMoreChars;
                }
    ...
    void ZepMode_Vim::HandleInsert(uint32_t key)
    {
        ...
        if (packCommand) {
            ...
            if (insertEnd > m_insertBegin) {
                ...
                auto strInserted = std::string(buffer.GetText().begin() + m_insertBegin,
                ...
                auto cursorAfterEscape =
                    std::max(lineBegin, m_insertBegin + BufferLocation(strInserted.length() - 1));

                auto cmd = std::make_shared<ZepCommand_Insert>(buffer, m_insertBegin, strInserted,
