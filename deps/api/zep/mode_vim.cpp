...
namespace Zep
{
CommandContext::CommandContext(const std::string &commandIn, ZepMode_Vim &md,
			       uint32_t lastK, uint32_t modifierK,
			       EditorMode editorMode)
	: owner(md), commandText(commandIn),
	  buffer(md.GetCurrentWindow()->GetBuffer()),
	  bufferCursor(md.GetCurrentWindow()->GetBufferCursor()),
	  tempReg("", false), lastKey(lastK), modifierKeys(modifierK),
	  mode(editorMode)
{
	registers.push('"');
	pRegister = &tempReg;
...
void CommandContext::UpdateRegisters()
{
	...
	if (op == CommandOperation::Delete ||
		...
		std::string str =
			std::string(buffer.GetText().begin() + beginRange,
				    buffer.GetText().begin() + endRange);

		// Delete commands fill up 1-9 registers
		if (command[0] == 'd' || command[0] == 'D') {
			for (int i = 9; i > 1; i--) {
				owner.GetEditor().SetRegister(
					'0' + (char)i,
					owner.GetEditor().GetRegister(
						'0' + (char)i - 1));
			}
			owner.GetEditor().SetRegister(
				'1',
				Register(str, (op ==
					       CommandOperation::DeleteLines)));
		}
...
void CommandContext::GetCommandAndCount()
{
	...
	if (itr != commandText.end()) {
		if (*itr == 'f' || *itr == 'F' || *itr == 'c') {
			while (itr != commandText.end() && std::isgraph(*itr)) {
				command1 += *itr;
				itr++;
			}
		} else {
...
void CommandContext::GetCommandRegisters()
{
	...
	if (commandWithoutCount[0] == '"' && commandWithoutCount.size() > 2) {
		...
		if (commandWithoutCount[1] == '_') {
			...
			if (owner.GetEditor().GetRegisters().find(
				    std::string({ reg })) !=
...
void ZepMode_Vim::ClampCursorForMode()
{
	...
	if (m_currentMode == EditorMode::Normal) {
		GetCurrentWindow()->SetBufferCursor(
			GetCurrentWindow()->GetBuffer().ClampToVisibleLine(
...
void ZepMode_Vim::SwitchMode(EditorMode mode)
{
	...
	if (mode == EditorMode::Insert && GetCurrentWindow() &&
	    GetCurrentWindow()->GetBuffer().TestFlags(FileFlags::Locked)) {
...
bool ZepMode_Vim::GetOperationRange(const std::string &op, EditorMode mode,
				    BufferLocation &beginRange,
				    BufferLocation &endRange) const
{
	...
	if (op == "visual") {
		...
		// Whole line
		beginRange = buffer.GetLinePos(bufferCursor,
					       LineLocation::LineBegin);
		endRange = buffer.GetLinePos(bufferCursor,
		...
		beginRange = bufferCursor;
		endRange = buffer.WordMotion(bufferCursor, SearchType::Word,
					     SearchDirection::Forward);
	} else if (op == "cw") {
		// Change word doesn't extend over the next space
		beginRange = bufferCursor;
		endRange =
			buffer.ChangeWordMotion(bufferCursor, SearchType::Word,
						SearchDirection::Forward);
	} else if (op == "cW") {
		beginRange = bufferCursor;
		endRange =
		...
		auto range =
			buffer.InnerWordMotion(bufferCursor, SearchType::Word);
		beginRange = range.first;
		endRange = range.second;
	} else if (op == "iW") {
		auto range =
...
bool ZepMode_Vim::HandleExCommand(const std::string &strCommand, const char key)
{
	...
	if (key == ExtKeys::RETURN) {
		auto pWindow =
			GetEditor().GetActiveTabWindow()->GetActiveWindow();
		auto &buffer = pWindow->GetBuffer();
		auto bufferCursor = pWindow->GetBufferCursor();
		if (GetEditor().Broadcast(std::make_shared<ZepMessage>(
			    Msg::HandleCommand, strCommand))) {
			m_currentCommand.clear();
			return true;
			...
			if (strTok.size() > 1) {
				if (strTok[1] == "%") {
					...
					auto fname = strTok[1];
					auto pBuffer =
						GetEditor().GetFileBuffer(
							fname);
					pTab->AddWindow(pBuffer, nullptr, true);
				}
			} else {
				pTab->AddWindow(
					GetEditor().GetEmptyBuffer("Empty"),
					nullptr, true);
			}
			...
			if (strTok.size() > 1) {
				...
				pTab->AddWindow(&GetEditor()
							 .GetActiveTabWindow()
							 ->GetActiveWindow()
							 ->GetBuffer(),
						pWindow, true);
			}
		} else if (strCommand.find(":hsplit") == 0 ||
			   strCommand.find(":split") == 0) {
			auto pTab = GetEditor().GetActiveTabWindow();
			auto strTok = string_split(strCommand, " ");
			if (strTok.size() > 1) {
				if (strTok[1] == "%") {
					...
					auto pBuffer =
						GetEditor().GetFileBuffer(
							fname);
					pTab->AddWindow(pBuffer, pWindow,
							false);
				}
			} else {
				pTab->AddWindow(&GetEditor()
			...
			if (strTok.size() > 1) {
				auto fname = strTok[1];
				GetCurrentWindow()->GetBuffer().SetFilePath(
					fname);
			}
			...
			if (m_currentMode == EditorMode::Visual) {
				auto range = GetCurrentWindow()
						     ->GetBuffer()
						     .GetSelection();
				start = range.first;
				end = range.second;
			} else {
				start = buffer.GetLinePos(
					bufferCursor,
					LineLocation::LineFirstGraphChar);
				end = buffer.GetLinePos(
					      bufferCursor,
					      LineLocation::LineLastGraphChar) +
				      1;
			}
			...
			case 1:
				...
				spMarker->description =
					"This is an example warning mark";
				break;
			case 0:
			default:
			...
			for (auto &editor_buffer : GetEditor().GetBuffers()) {
				if (!editor_buffer->GetName().empty()) {
					std::string displayText =
						editor_buffer->GetName();
					displayText = string_replace(
						displayText, "\n", "^J");
					if (&GetCurrentWindow()->GetBuffer() ==
					    editor_buffer.get()) {
						str << "*";
					} else {
						str << " ";
					}
					if (editor_buffer->TestFlags(
						    FileFlags::Dirty)) {
						str << "+";
					} else {
						str << " ";
			...
			if (strTok.size() > 1) {
				try {
					...
					auto current = 0;
					for (auto &editor_buffer :
					     GetEditor().GetBuffers()) {
						if (index == current) {
							GetCurrentWindow()->SetBuffer(
								editor_buffer
									.get());
						}
						current++;
...
bool ZepMode_Vim::GetCommand(CommandContext &context)
{
	...
	if ((context.modifierKeys & ModifierKey::Ctrl) &&
	    HandleGlobalCtrlCommand(context.command, context.modifierKeys,
				    needMoreChars)) {
		if (needMoreChars) {
			context.commandResult.flags |=
				CommandResultFlags::NeedMoreChars;
			return false;
		} else {
			return true;
		}
	...
	// Motion
		...
		GetCurrentWindow()->SetBufferCursor(context.buffer.GetLinePos(
			bufferCursor, LineLocation::LineLastNonCR));
		return true;
	} else if (context.command == "0") {
		GetCurrentWindow()->SetBufferCursor(context.buffer.GetLinePos(
			bufferCursor, LineLocation::LineBegin));
		return true;
	} else if (context.command == "^") {
		GetCurrentWindow()->SetBufferCursor(context.buffer.GetLinePos(
			bufferCursor, LineLocation::LineFirstGraphChar));
		return true;
	}
	// Moving between tabs
	else if (context.command == "H" &&
		 (context.modifierKeys & ModifierKey::Shift)) {
		GetEditor().PreviousTabWindow();
		return true;
	} else if (context.command == "L" &&
		   (context.modifierKeys & ModifierKey::Shift)) {
		GetEditor().NextTabWindow();
		return true;
	} else if (context.command == "j" || context.command == "+" ||
		   context.lastKey == ExtKeys::DOWN) {
		GetCurrentWindow()->MoveCursorY(context.count);
		context.commandResult.flags |= CommandResultFlags::HandledCount;
		return true;
	} else if (context.command == "k" || context.command == "-" ||
		   context.lastKey == ExtKeys::UP) {
		GetCurrentWindow()->MoveCursorY(-context.count);
		context.commandResult.flags |= CommandResultFlags::HandledCount;
		return true;
	} else if (context.command == "l" ||
		   context.lastKey == ExtKeys::RIGHT) {
		auto lineEnd = context.buffer.GetLinePos(
			context.bufferCursor, LineLocation::LineLastNonCR);
		GetCurrentWindow()->SetBufferCursor(std::min(
			context.bufferCursor + context.count, lineEnd));
		context.commandResult.flags |= CommandResultFlags::HandledCount;
		return true;
	} else if (context.command == "h" || context.lastKey == ExtKeys::LEFT) {
		auto lineStart = context.buffer.GetLinePos(
			context.bufferCursor, LineLocation::LineBegin);
		GetCurrentWindow()->SetBufferCursor(std::max(
			context.bufferCursor - context.count, lineStart));
		context.commandResult.flags |= CommandResultFlags::HandledCount;
		return true;
	} else if ((context.command == "f" &&
		    (context.modifierKeys & ModifierKey::Ctrl)) ||
		   context.lastKey == ExtKeys::PAGEDOWN) {
		...
		GetCurrentWindow()->MoveCursorY(
			(GetCurrentWindow()->GetMaxDisplayLines() - 2) *
			context.count);
		context.commandResult.flags |= CommandResultFlags::HandledCount;
		return true;
	} else if ((context.command == "d" &&
		    (context.modifierKeys & ModifierKey::Ctrl)) ||
		   context.lastKey == ExtKeys::PAGEDOWN) {
		// Note: the vim spec says 'half visible lines' for up/down
		GetCurrentWindow()->MoveCursorY(
			(GetCurrentWindow()->GetNumDisplayedLines() / 2) *
			context.count);
		context.commandResult.flags |= CommandResultFlags::HandledCount;
		return true;
	} else if ((context.command == "b" &&
		    (context.modifierKeys & ModifierKey::Ctrl)) ||
		   context.lastKey == ExtKeys::PAGEUP) {
		// Note: the vim spec says 'visible lines - 2' for a 'page'
		GetCurrentWindow()->MoveCursorY(
			-(GetCurrentWindow()->GetMaxDisplayLines() - 2) *
			context.count);
		...
		if (context.foundCount) {
			...
			count = std::min(context.buffer.GetLineCount() - 1,
					 count);
			if (count < 0)
			...
			auto lastLine = context.buffer.GetLinePos(
				MaxCursorMove, LineLocation::LineBegin);
			GetCurrentWindow()->SetBufferCursor(lastLine);
			context.commandResult.flags |=
				CommandResultFlags::HandledCount;
		}
		return true;
		...
		if (context.mode == EditorMode::Insert) {
			...
			if (loc > 0)
				context.beginRange =
					context.buffer.LocationFromOffsetByChars(
						loc, -1);
			context.endRange =
				context.buffer.LocationFromOffsetByChars(loc,
									 0);
		...
		auto target =
			context.buffer.WordMotion(context.bufferCursor,
						  SearchType::Word,
						  SearchDirection::Forward);
		...
		auto target =
			context.buffer.EndWordMotion(context.bufferCursor,
						     SearchType::Word,
						     SearchDirection::Forward);
		GetCurrentWindow()->SetBufferCursor(target);
		return true;
		...
		if (context.command == "g") {
			...
			auto target = context.buffer.EndWordMotion(
				context.bufferCursor, SearchType::Word,
				SearchDirection::Backward);
			GetCurrentWindow()->SetBufferCursor(target);
			return true;
		} else if (context.command == "gE") {
		...
		context.beginRange = context.buffer.GetLinePos(
			context.bufferCursor, LineLocation::LineCRBegin);
		context.endRange = context.buffer.GetLinePos(
			context.bufferCursor, LineLocation::BeyondLineEnd);
		context.op = CommandOperation::Delete;
		...
		if (m_currentMode == EditorMode::Visual) {
			...
			if (context.command == "V") {
				m_visualBegin = context.buffer.GetLinePos(
					context.bufferCursor,
					LineLocation::LineBegin);
				m_visualEnd = context.buffer.GetLinePos(
					context.bufferCursor,
					LineLocation::BeyondLineEnd);
			} else {
		...
		if (m_currentMode == EditorMode::Visual) {
			...
			if (context.command != "x" ||
			    std::isgraph(context.buffer.GetText()[loc]) ||
			    std::isblank(context.buffer.GetText()[loc])) {
				context.beginRange = loc;
				context.endRange = std::min(
					context.buffer.GetLinePos(
						loc, LineLocation::LineCRBegin),
					context.buffer.LocationFromOffsetByChars(
						loc, context.count));
				context.op = CommandOperation::Delete;
				context.commandResult.flags |=
		...
		if (context.command == "d") {
			...
			if (GetOperationRange("visual", context.mode,
					      context.beginRange,
					      context.endRange)) {
				context.op = CommandOperation::Delete;
				context.commandResult.modeSwitch =
					EditorMode::Normal;
			} else {
				context.commandResult.flags |=
					CommandResultFlags::NeedMoreChars;
			...
			if (GetOperationRange("line", context.mode,
					      context.beginRange,
					      context.endRange)) {
			...
			if (GetOperationRange("$", context.mode,
					      context.beginRange,
					      context.endRange)) {
				context.op = CommandOperation::Delete;
			}
		} else if (context.command == "dw") {
			if (GetOperationRange("w", context.mode,
					      context.beginRange,
					      context.endRange)) {
				context.op = CommandOperation::Delete;
			}
			...
			if (GetOperationRange("aw", context.mode,
					      context.beginRange,
					      context.endRange)) {
				context.op = CommandOperation::Delete;
			}
			...
			if (GetOperationRange("iw", context.mode,
					      context.beginRange,
					      context.endRange)) {
				context.op = CommandOperation::Delete;
			}
	...
	// Substitute
		...
		if (context.command == "S") {
			...
			else if (GetOperationRange("cursor", context.mode,
						   context.beginRange,
						   context.endRange)) {
				context.op = CommandOperation::Delete;
			}
		...
		if (context.command == "c") {
			...
			if (GetOperationRange("cw", context.mode,
					      context.beginRange,
					      context.endRange)) {
				context.op = CommandOperation::Delete;
			}
		...
		if (context.mode == EditorMode::Visual) {
			...
			context.endRange = m_visualEnd;
			context.commandResult.modeSwitch = EditorMode::Normal;
			context.op = m_lineWise ? CommandOperation::CopyLines :
							CommandOperation::Copy;
		...
		GetCurrentWindow()->SetBufferCursor(context.buffer.GetLinePos(
			bufferCursor, LineLocation::LineCRBegin));
		context.commandResult.modeSwitch = EditorMode::Insert;
		return true;
	} else if (context.command == "I") {
		...
		if (!m_lastFind.empty()) {
			GetCurrentWindow()->SetBufferCursor(
				context.buffer.FindOnLineMotion(
					bufferCursor,
					(const utf8 *)m_lastFind.c_str(),
					m_lastFindDirection));
			return true;
		}
	...
	if (context.op == CommandOperation::Delete ||
	    context.op == CommandOperation::DeleteLines) {
		auto cmd = std::make_shared<ZepCommand_DeleteRange>(
			context.buffer, context.beginRange, context.endRange,
			context.bufferCursor);
		context.commandResult.spCommand =
			std::static_pointer_cast<ZepCommand>(cmd);
		return true;
	} else if (context.op == CommandOperation::Insert &&
		   !context.pRegister->text.empty()) {
		auto cmd = std::make_shared<ZepCommand_Insert>(
			context.buffer, context.beginRange,
			context.pRegister->text, context.bufferCursor,
			context.cursorAfterOverride);
		context.commandResult.spCommand =
			std::static_pointer_cast<ZepCommand>(cmd);
		return true;
	} else if (context.op == CommandOperation::Copy ||
		   context.op == CommandOperation::CopyLines) {
...
void ZepMode_Vim::UpdateVisualSelection()
{
	...
	if (m_currentMode == EditorMode::Visual) {
		...
		if (m_lineWise) {
			m_visualEnd =
				GetCurrentWindow()->GetBuffer().GetLinePos(
					GetCurrentWindow()->GetBufferCursor(),
					LineLocation::BeyondLineEnd);
		} else {
			m_visualEnd =
				GetCurrentWindow()
					->GetBuffer()
					.LocationFromOffsetByChars(
						GetCurrentWindow()
							->GetBufferCursor(),
						1);
		}
		GetCurrentWindow()->GetBuffer().SetSelection(
			BufferRange{ m_visualBegin, m_visualEnd });
	}
}
void ZepMode_Vim::AddKeyPress(uint32_t key, uint32_t modifierKeys)
{
	...
	if (m_currentMode == EditorMode::Normal ||
		...
		if (m_currentCommand[0] == ':' || m_currentCommand[0] == '/' ||
		...
		CommandContext context(m_currentCommand, *this, key,
				       modifierKeys, m_currentMode);
		if (GetCommand(context)) {
			...
			if (context.commandResult.spCommand) {
				if (key == '.' && !m_lastInsertString.empty() &&
				    context.commandResult.modeSwitch ==
					    EditorMode::Insert) {
				...
				if (appendDotInsert ||
				    (context.count > 1 &&
				     !(context.commandResult.flags &
				       CommandResultFlags::HandledCount))) {
					context.commandResult.spCommand->SetFlags(
						CommandFlags::GroupBoundary);
				}
			...
			if (!(context.commandResult.flags &
			      CommandResultFlags::HandledCount)) {
				for (int i = 1; i < context.count; i++) {
					...
					CommandContext contextInner(
						m_currentCommand, *this, key,
						modifierKeys, m_currentMode);
					if (GetCommand(contextInner) &&
					    contextInner.commandResult
						    .spCommand) {
						// Group counted
			...
			if (appendDotInsert) {
				if (!m_lastInsertString.empty()) {
					...
					cmd->SetFlags(
						CommandFlags::GroupBoundary);
					AddCommand(std::static_pointer_cast<
						   ZepCommand>(cmd));
				}
				SwitchMode(EditorMode::Normal);
			}
			...
			if (!m_currentCommand.empty() &&
			    m_currentCommand.find_first_not_of("0123456789") ==
				    std::string::npos) {
				context.commandResult.flags |=
					CommandResultFlags::NeedMoreChars;
			}
...
void ZepMode_Vim::HandleInsert(uint32_t key)
{
	...
	if (m_pendingEscape) {
		...
		auto canEscape =
			timer_get_elapsed_seconds(m_insertEscapeTimer) < .25f;
		if (canEscape && key == 'k') {
			packCommand = true;
			key = ExtKeys::ESCAPE;
		}
	...
	if (packCommand) {
		...
		if (insertEnd > m_insertBegin) {
			...
			auto strInserted = std::string(
				buffer.GetText().begin() + m_insertBegin,
				buffer.GetText().begin() + insertEnd);

			// Remember the inserted string for repeating the command
			m_lastInsertString = strInserted;

			auto lineBegin = buffer.GetLinePos(
				bufferCursor, LineLocation::LineBegin);
			...
			auto cursorAfterEscape = std::max(
				lineBegin,
				m_insertBegin +
					BufferLocation(strInserted.length() -
						       1));

			auto cmd = std::make_shared<ZepCommand_Insert>(
				buffer, m_insertBegin, strInserted,
				m_insertBegin, cursorAfterEscape);
		...
		if (key == ExtKeys::ESCAPE) {
			...
			CommandContext context("", *this, key, 0,
					       EditorMode::Insert);
	...
	if (key == 'j' && !m_pendingEscape) {
		...
		GetCurrentWindow()->SetBufferCursor(bufferCursor +
						    long(ch.size()));
	}
}
...
void ZepMode_Vim::PreDisplay()
{
	...
	if (m_pendingEscape &&
	    timer_get_elapsed_seconds(m_insertEscapeTimer) > .25f) {
		m_pendingEscape = false;
		GetCurrentWindow()->GetBuffer().Insert(
			GetCurrentWindow()->GetBufferCursor(), "j");
		GetCurrentWindow()->SetBufferCursor(
			GetCurrentWindow()->GetBufferCursor() + 1);
	}
}
} // namespace Zep
