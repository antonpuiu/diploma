...
void ZepMode_Standard::AddKeyPress(uint32_t key, uint32_t modifierKeys)
{
	...
	BufferLocation endOffset = buffer.LocationFromOffsetByChars(
		bufferCursor, long(ch.length()));

	enum class CommandOperation { None, Delete, Insert, Copy, Paste };
	...
	if (modifierKeys & ModifierKey::Ctrl) {
		...
		if (HandleGlobalCtrlCommand(keyCache, modifierKeys,
					    needMoreChars)) {
			if (!needMoreChars) {
				keyCache.clear();
			}
		...
		// Motions fall through to selection code
			...
			auto target = buffer.StandardCtrlMotion(
				bufferCursor, SearchDirection::Forward);
			GetCurrentWindow()->SetBufferCursor(target.second);
		} else if (key == ExtKeys::LEFT) {
			auto target = buffer.StandardCtrlMotion(
				bufferCursor, SearchDirection::Backward);
			GetCurrentWindow()->SetBufferCursor(target.second);
		...
		// Cut/Copy
			...
			if (m_currentMode == EditorMode::Visual) {
				...
				lineWise = true;
				startOffset = buffer.GetLinePos(
					bufferCursor, LineLocation::LineBegin);
				endOffset = buffer.GetLinePos(
					bufferCursor,
		...
		// Beginning of line
		auto pos = buffer.GetLinePos(bufferCursor,
		...
		if (m_currentMode == EditorMode::Visual) {
			...
			endOffset = buffer.LocationFromOffsetByChars(
				startOffset, 1);
		}
		...
		if (m_currentMode == EditorMode::Visual) {
			...
			if (startOffset > 0)
				startOffset = buffer.LocationFromOffsetByChars(
					startOffset, -1);
		}
	} else {
		op = CommandOperation::Insert;
	...
	if (m_currentMode == EditorMode::Visual) {
		if (begin_shift) {
			if (GetCurrentWindow()->GetBufferCursor() <=
			    startOffset) {
				m_visualBegin =
		...
		if (GetCurrentWindow()->GetBufferCursor() > m_visualBegin) {
			...
			m_visualEnd =
				GetCurrentWindow()
					->GetBufferCursor(); // buffer.LocationFromOffsetByChars(GetCurrentWindow()->GetBufferCursor(), -1);
		}
	}
	...
	if (copyRegion || op == CommandOperation::Copy) {
		// Grab it
		std::string str =
			std::string(buffer.GetText().begin() + startOffset,
				    buffer.GetText().begin() + endOffset);
