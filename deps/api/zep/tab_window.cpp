...
namespace Zep
{
    ...
    ZepWindow* ZepTabWindow::DoMotion(WindowMotion motion)
    {
        ...
        if (pBestRegion != nullptr) {
            auto itrFound = std::find_if(m_windowRegions.begin(), m_windowRegions.end(),
        ...
        if (pParent == nullptr) {
            ...
            if (pParentRegion->vertical == vsplit) {
                ...
                auto itrFound = std::find_if(pParentRegion->children.begin(),
                                             pParentRegion->children.end(), [&](auto pCurrent) {
    ...
    void ZepTabWindow::RemoveWindow(ZepWindow* pWindow)
    {
        ...
        std::function<void(std::shared_ptr<Region>, std::shared_ptr<Region>)> fnRemoveRegion =
            [&](auto parent, auto child) {
                ...
                if (parent) {
                    auto itrFound =
                        std::find_if(parent->children.begin(), parent->children.end(),
                                     [&child](auto pCurrent) { return pCurrent == child; });
                ...
                if (parent && parent->children.size() == 1) {
                    if (parent != m_spRootRegion) {
                        ...
                        auto itrWin = std::find_if(m_windowRegions.begin(), m_windowRegions.end(),
                                                   [&](auto pr) {
