...
class ZepDisplay {
	...
	virtual uint32_t GetCodePointCount(const uint8_t *pCh,
					   const uint8_t *pEnd) const;
	virtual void DrawRect(const NRectf &rc,
			      const NVec4f &col = NVec4f(1.0f)) const;
	...
	virtual const NVec2f &GetPixelScale() const;
	...
	virtual ZepFont &GetFont(ZepTextType type) override
	{
		if (m_fonts[(int)type] == nullptr) {
			if (m_spDefaultFont == nullptr) {
				m_spDefaultFont =
					std::make_shared<ZepFontNull>(*this);
			}
