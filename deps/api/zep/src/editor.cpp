...
ZepEditor::ZepEditor(ZepDisplay *pDisplay, const ZepPath &configRoot,
		     uint32_t flags, IZepFileSystem *pFileSystem)
...
ZepEditor::~ZepEditor()
{
	std::for_each(m_tabWindows.begin(), m_tabWindows.end(),
		      [](ZepTabWindow *w) { delete w; });
	m_tabWindows.clear();
	delete m_pDisplay;
...
void ZepEditor::LoadConfig(const ZepPath &config_path)
{
	...
	try {
		...
		std::ostringstream str;
		str << config_path.filename().string()
		    << " : Failed to parse. ";
		SetCommandText(str.str());
	}
}
...
void ZepEditor::LoadConfig(std::shared_ptr<cpptoml::table> spConfig)
{
	try {
		m_config.showNormalModeKeyStrokes =
			spConfig->get_qualified_as<bool>(
					"editor.show_normal_mode_keystrokes")
				.value_or(false);
		m_config.showIndicatorRegion =
			spConfig->get_qualified_as<bool>(
					"editor.show_indicator_region")
				.value_or(true);
		...
		m_config.autoHideCommandRegion =
			spConfig->get_qualified_as<bool>(
					"editor.autohide_command_region")
				.value_or(false);
		m_config.cursorLineSolid =
			spConfig->get_qualified_as<bool>(
					"editor.cursor_line_solid")
				.value_or(true);
		m_config.backgroundFadeTime =
			(float)spConfig
				->get_qualified_as<double>(
					"editor.background_fade_time")
				.value_or(60.0f);
		m_config.backgroundFadeWait =
			(float)spConfig
				->get_qualified_as<double>(
					"editor.background_fade_wait")
				.value_or(60.0f);
		m_config.showScrollBar =
			spConfig->get_qualified_as<uint32_t>(
					"editor.show_scrollbar")
				.value_or(1);
		m_config.lineMargins.x =
			(float)spConfig
				->get_qualified_as<double>(
					"editor.line_margin_top")
				.value_or(1);
		m_config.lineMargins.y =
			(float)spConfig
				->get_qualified_as<double>(
					"editor.line_margin_bottom")
				.value_or(1);
		m_config.widgetMargins.x =
			(float)spConfig
				->get_qualified_as<double>(
					"editor.widget_margin_top")
				.value_or(1);
		m_config.widgetMargins.y =
			(float)spConfig
				->get_qualified_as<double>(
					"editor.widget_margin_bottom")
				.value_or(1);
		m_config.shortTabNames =
			spConfig->get_qualified_as<bool>(
					"editor.short_tab_names")
				.value_or(false);
		auto styleStr = string_tolower(
			spConfig->get_qualified_as<std::string>("editor.style")
				.value_or("normal"));
		if (styleStr == "normal") {
...
void ZepEditor::SaveConfig(std::shared_ptr<cpptoml::table> spConfig)
{
	...
	table->insert("style", m_config.style == EditorStyle::Minimal ?
					     "minimal" :
					     "normal");
...
void ZepEditor::SaveBuffer(ZepBuffer &buffer)
{
	...
	if (buffer.HasFileFlags(FileFlags::ReadOnly)) {
		...
		if (!buffer.Save(size)) {
			...
			strText << "Wrote " << buffer.GetFilePath().string()
				<< ", " << size << " bytes";
		}
	}
...
std::vector<ZepWindow *>
ZepEditor::FindBufferWindows(const ZepBuffer *pBuffer) const
{
...
ZepBuffer *ZepEditor::GetEmptyBuffer(const std::string &name,
				     uint32_t fileFlags)
{
...
ZepBuffer *ZepEditor::GetFileBuffer(const ZepPath &filePath, uint32_t fileFlags,
				    bool create)
{
	auto path = GetFileSystem().Exists(filePath) ?
				  GetFileSystem().Canonical(filePath) :
				  filePath;
	if (!path.empty()) {
		for (auto &pBuffer : m_buffers) {
			if (!pBuffer->GetFilePath().empty()) {
				if (GetFileSystem().Equivalent(
					    pBuffer->GetFilePath(), path)) {
					return pBuffer.get();
				}
			}
		}
...
ZepWindow *ZepEditor::AddTree()
{
	auto pTree = GetEmptyBuffer("Tree.tree",
				    FileFlags::Locked | FileFlags::ReadOnly);
	auto pTreeWindow = GetActiveTabWindow()->AddWindow(
		pTree, nullptr, RegionLayoutType::HBox);
	...
	pRoot->AddChild(std::make_shared<ZepFileNode>(
		"Child1", ZepTreeNodeFlags::IsFolder));
	auto pChild2 = pRoot->AddChild(std::make_shared<ZepFileNode>(
		"Child2", ZepTreeNodeFlags::IsFolder));
	pChild2->AddChild(std::make_shared<ZepFileNode>("Child2_1"));

	pRoot->ExpandAll(true);

	auto pMode = std::make_shared<ZepMode_Tree>(
		*this, pTreeModel, *pActiveWindow, *pTreeWindow);
	pTree->SetMode(pMode);
...
ZepWindow *ZepEditor::AddSearch()
{
	...
	auto pSearchBuffer = GetEmptyBuffer(
		"Search", FileFlags::Locked | FileFlags::ReadOnly);
	pSearchBuffer->SetBufferType(BufferType::Search);
	pSearchBuffer->SetSyntax(std::make_shared<ZepSyntax>(
		*pSearchBuffer, search_keywords, search_identifiers,
		ZepSyntaxFlags::CaseInsensitive));

	auto pActiveWindow = GetActiveTabWindow()->GetActiveWindow();

	bool hasGit = false;
	auto searchPath = GetFileSystem().GetSearchRoot(
		pActiveWindow->GetBuffer().GetFilePath(), hasGit);

	auto pSearchWindow = GetActiveTabWindow()->AddWindow(
		pSearchBuffer, nullptr, RegionLayoutType::VBox);
	pSearchWindow->SetWindowFlags(pSearchWindow->GetWindowFlags() |
				      WindowFlags::Modal);

	auto pMode = std::make_shared<ZepMode_Search>(
		*this, *pActiveWindow, *pSearchWindow, searchPath);
...
ZepBuffer *ZepEditor::InitWithText(const std::string &strName,
...
void ZepEditor::UpdateWindowState()
{
	...
	if (m_pActiveTabWindow && !m_pActiveTabWindow->GetActiveWindow()) {
		if (!m_pActiveTabWindow->GetWindows().empty()) {
			m_pActiveTabWindow->SetActiveWindow(
				m_pActiveTabWindow->GetWindows().back());
...
{
	auto itr = std::find(m_tabWindows.begin(), m_tabWindows.end(),
			     m_pActiveTabWindow);
...
void ZepEditor::UpdateTabs()
{
	...
	if (GetTabWindows().size() > 1) {
		...
		for (auto &window : GetTabWindows()) {
			...
			auto tabColor =
				GetTheme().GetColor(ThemeColor::TabActive);
			if (buffer.HasFileFlags(FileFlags::HasWarnings)) {
				tabColor = GetTheme().GetColor(
					ThemeColor::Warning);
			}
			...
			if (buffer.HasFileFlags(FileFlags::HasErrors)) {
				tabColor =
					GetTheme().GetColor(ThemeColor::Error);
			}
			...
			auto tabLength =
				m_pDisplay->GetFont(ZepTextType::Text)
					.GetTextSize(
			...
			spTabRegionTab->padding =
				DPI_VEC2(NVec2f(textBorder, textBorder));
...
ZepTabWindow *ZepEditor::AddTabWindow()
{
	...
	auto pEmpty =
		GetEmptyBuffer("[No ExCommandName]", FileFlags::DefaultBuffer);
...
{
	Broadcast(
		std::make_shared<ZepMessage>(Msg::RequestQuit, "RequestQuit"));
}
...
void ZepEditor::RemoveTabWindow(ZepTabWindow *pTabWindow)
{
	...
	if (m_tabWindows.empty()) {
		...
		if (m_pActiveTabWindow == pTabWindow) {
			m_pActiveTabWindow =
				m_tabWindows[m_tabWindows.size() - 1];
...
void ZepEditor::RegisterBufferMode(const std::string &extension,
				   std::shared_ptr<ZepMode> spMode)
{
...
void ZepEditor::SetBufferMode(ZepBuffer &buffer) const
{
	...
	std::string ext;
	std::string fileName;
	if (buffer.GetFilePath().has_filename() &&
	    buffer.GetFilePath().filename().has_extension()) {
		ext = string_tolower(
			buffer.GetFilePath().filename().extension().string());
		fileName = string_tolower(
			buffer.GetFilePath().filename().string());
	} else {
		auto str = buffer.GetName();
		size_t dot_pos = str.find_last_of(".");
		if (dot_pos != std::string::npos) {
			ext = string_tolower(
				str.substr(dot_pos, str.length() - dot_pos));
		}
	}
	...
	if (pMsg->messageId == Msg::MouseDown) {
		for (auto &tab : m_tabRegion->children) {
			if (tab->rect.Contains(pMsg->pos)) {
				auto pTabRegionTab =
					std::static_pointer_cast<TabRegionTab>(
						tab);
...
{
	m_lastCursorBlink =
		(int(timer_get_elapsed_seconds(m_cursorTimer) * 1.75f) & 1) ?
			      true :
			      false;
	return m_lastCursorBlink;
}

void ZepEditor::SetDisplayRegion(const NVec2f &topLeft,
				 const NVec2f &bottomRight)
{
...
void ZepEditor::UpdateSize()
{
	...
	const float commandSize = uiFont.GetPixelHeight() * commandCount +
				  DPI_X(textBorder) * 2.0f;
	...
	if (GetTabWindows().size() > 1) {
		m_tabRegion->fixed_size = NVec2f(
			0.0f, uiFont.GetPixelHeight() + DPI_X(textBorder) * 2);
	...
	if (GetActiveTabWindow()) {
		GetActiveTabWindow()->SetDisplayRegion(
			m_tabContentRegion->rect);
	}
}
...
void ZepEditor::Display()
{
	...
	if (GetConfig().style == EditorStyle::Normal) {
		GetDisplay().DrawRectFilled(
			m_editorRegion->rect,
			GetTheme().GetColor(ThemeColor::Background));
	}

	// Background rect for CommandLine
	if (!GetCommandText().empty() ||
	    (GetConfig().autoHideCommandRegion == false)) {
		m_pDisplay->DrawRectFilled(
			m_commandRegion->rect,
			GetTheme().GetColor(ThemeColor::Background));
	}

	// Draw command text
	auto screenPosYPx = m_commandRegion->rect.topLeftPx +
			    NVec2f(0.0f, DPI_X(textBorder));
	for (int i = 0; i < commandSpace; i++) {
		if (!commandLines[i].empty()) {
			auto textSize = uiFont.GetTextSize(
				(const uint8_t *)commandLines[i].c_str(),
				(const uint8_t *)commandLines[i].c_str() +
					commandLines[i].size());
			m_pDisplay->DrawChars(
				uiFont, screenPosYPx,
	...
	if (GetConfig().style == EditorStyle::Normal) {
		...
		m_pDisplay->DrawRectFilled(
			NRectf(NVec2f(m_tabRegion->rect.Left(),
				      m_tabRegion->rect.Bottom() - DPI_Y(1)),
			       NVec2f(m_tabRegion->rect.Right(),
				      m_tabRegion->rect.Bottom())),
	...
	NRectf tabRect;
	for (auto &tab : m_tabRegion->children) {
		if (std::static_pointer_cast<TabRegionTab>(tab)->pTabWindow ==
		    pActiveTabWindow) {
			tabRect = tab->rect;
			break;
		}
	...
	if (tabRect.Width() != 0.0f) {
		if ((tabRect.Left() - tabRect.Width() + m_tabOffsetX) <
		    m_tabRegion->rect.Left()) {
			m_tabOffsetX += m_tabRegion->rect.Left() -
					(tabRect.Left() + m_tabOffsetX -
					 tabRect.Width());
		} else if ((tabRect.Right() + m_tabOffsetX + tabRect.Width()) >
			   m_tabRegion->rect.Right()) {
			m_tabOffsetX -=
				(tabRect.Right() + m_tabOffsetX -
				 m_tabRegion->rect.Right() + tabRect.Width());
		}
	}

	// Clamp it
	m_tabOffsetX = std::min(m_tabOffsetX, 0.0f);
	m_tabOffsetX = std::max(std::min(tabRegionSize - virtualSize, 0.0f),
				m_tabOffsetX);

	// Now display the tabs
	for (auto &tab : m_tabRegion->children) {
		auto spTabRegionTab =
			std::static_pointer_cast<TabRegionTab>(tab);
		...
		m_pDisplay->DrawChars(
			...
			textCol, (const uint8_t *)spTabRegionTab->name.c_str());
	}
...
{
	m_mousePos = mousePos;
	bool handled = Broadcast(
		std::make_shared<ZepMessage>(Msg::MouseMove, mousePos));
	m_bPendingRefresh = true;
	return handled;
}
...
bool ZepEditor::OnMouseDown(const NVec2f &mousePos, ZepMouseButton button)
{
	m_mousePos = mousePos;
	bool handled = Broadcast(
		std::make_shared<ZepMessage>(Msg::MouseDown, mousePos, button));
	m_bPendingRefresh = true;
	return handled;
}
...
bool ZepEditor::OnMouseUp(const NVec2f &mousePos, ZepMouseButton button)
{
	m_mousePos = mousePos;
	bool handled = Broadcast(
		std::make_shared<ZepMessage>(Msg::MouseUp, mousePos, button));
	m_bPendingRefresh = true;
	return handled;
}
