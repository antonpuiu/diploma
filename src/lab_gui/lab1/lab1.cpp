...
void Lab1::Update(float deltaTimeSeconds)
{
    ...
    if (zep->spEditor->GetTabWindows().empty()) {
        window->Close();
        showZep = false;
    }
    ...
    if (showZep) {
        if (ImGui::BeginMainMenuBar()) {
            ...
            if (ImGui::BeginMenu("Settings")) {
                if (ImGui::BeginMenu("Editor Mode")) {
                    bool enabledVim = strcmp(zep->GetEditor().GetCurrentMode()->Name(),
                                             Zep::ZepMode_Vim::StaticName()) == 0;
                ...
                if (ImGui::BeginMenu("Theme")) {
                    bool enabledDark =
                        zep->GetEditor().GetTheme().GetThemeType() == ThemeType::Dark ? true :
                                                                                        false;
            ...
            if (ImGui::BeginMenu("Window")) {
                ...
                if (ImGui::MenuItem("Horizontal Split")) {
                    pTabWindow->AddWindow(&pTabWindow->GetActiveWindow()->GetBuffer(),
                                          pTabWindow->GetActiveWindow(), false);
                } else if (ImGui::MenuItem("Vertical Split")) {
                    pTabWindow->AddWindow(&pTabWindow->GetActiveWindow()->GetBuffer(),
                                          pTabWindow->GetActiveWindow(), true);
                }
                ImGui::EndMenu();
            }
            ...
            if (ImGui::BeginMenu("Timings")) {
                for (auto& p : globalProfiler.timerData) {
                    ...
                    strval << p.first << " : " << p.second.current / 1000.0
        ...
        zep->spEditor->SetDisplayRegion(toNVec2f(ImGui::GetWindowPos()),
