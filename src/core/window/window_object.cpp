...
void WindowObject::FullScreen()
{
	...
	const GLFWvidmode *videoDisplay = glfwGetVideoMode(monitor);
	window->handle =
		glfwCreateWindow(videoDisplay->width, videoDisplay->height,
...
void WindowObject::WindowMode()
{
	...
	if (props.centered) {
		...
		const GLFWvidmode *videoDisplay = glfwGetVideoMode(monitor);
		auto screenSize =
			glm::ivec2(videoDisplay->width, videoDisplay->height);
...
void WindowObject::UpdateObservers()
{
	...
	if (resizeEvent) {
		...
		for (auto obs : observers) {
			obs->OnWindowResize(props.resolution.x,
					    props.resolution.y);
		}
	}
	...
	if (mouseMoveEvent) {
		mouseMoveEvent = false;
		for (auto obs : observers) {
			obs->OnMouseMove(props.cursorPos.x, props.cursorPos.y,
	...
	if (pressEvent) {
		for (auto obs : observers) {
			obs->OnMouseBtnPress(props.cursorPos.x,
					     props.cursorPos.y, pressEvent,
					     keyMods);
		}
	}
	...
	if (releaseEvent) {
		for (auto obs : observers) {
			obs->OnMouseBtnRelease(props.cursorPos.x,
					       props.cursorPos.y, releaseEvent,
					       keyMods);
		}
	}
